...
df <- df %>%
  mutate(pass = ifelse(final_result == "Pass" | final_result == "Distinction", 1, 0),
         total_as = as_sub_34873 + as_sub_34874) %>%
  select(id_student, code_module, code_presentation, date_registration, date_unregistration, final_result, pass,
...
## Preliminary model
```{r}
m1 <- glm(pass ~ days + log(total_vle) + log(unique_vle) + as_sub_34873 + as_sub_34874, data = df)
summary(m1)
```

```{r}
m1 <- glm(pass ~ days + log(total_vle) + log(unique_vle) + as_scr_34873 + as_scr_34874, data = df)
summary(m1)
```
...
```{r}
# See https://tbradley1013.github.io/2018/02/01/pca-in-a-tidy-verse-framework/
x <- stdinf_f13j_s_vle %>%
  filter(date <= 50) %>%
  ...
  ungroup() %>%
  mutate(id_site = paste0("id", id_site))
```
...
pr.out <- prcomp(z, scale=TRUE)

a <- as.data.frame(pr.out$rotation)
...
```{r}
df <- inner_join(df, df_pca5, by = "id_student")
```

```{r}
df %>%
  ggplot() +
  geom_boxplot(aes(x = final_result, y = PC1, fill = final_result)) 
```
